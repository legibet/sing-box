...
func (c *streamConn) Close() error {
	c.closeOnce.Do(func() {
		...
		// Deadlines live on the shared TLS conn and would fire on the next
		// stream. If a leftover deadline cannot be cleared, drop the conn.
		if reusable && c.conn.SetDeadline(time.Time{}) != nil {
			reusable = false
		}
