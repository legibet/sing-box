...
func (h *Outbound) SetKeepIdleConnections(keep bool) {
	h.pool.SetKeepIdleConnections(keep)
}
...
// connection if one is available and otherwise dialling a fresh one. A pooled
// connection that fails the header write is dropped and replaced by one new
// dial; a freshly dialled connection is not retried.
func (h *Outbound) dialStream(ctx context.Context, destination M.Socksaddr) (net.Conn, error) {
	...
	err = h.openStream(stream, destination, withAuth)
	if err != nil {
		common.Close(stream)
		if withAuth {
			return nil, err
		...
		stream = h.wrapStream(conn)
		if err = h.openStream(stream, destination, true); err != nil {
			common.Close(stream)
			return nil, err
		}
	}
